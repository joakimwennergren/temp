...
public class Level1 : BaseLevel
{
    ...
    private MushroomSpawner _MushroomSpawner;

    private List<Position> positions = new List<Position>()
    {
        // Layer 1 positions
        new Position(25.0f, 11.0f, 35.0f),
        new Position(35.0f, 10.0f, 35.0f),
        new Position(45.0f, 11.0f, 35.0f),
    ...
    public Level1()
    {
        Flare.Set(new Position(0.0f, 0.0f, 100.0f));
        Flare.Set(new Dimension(0.0f, 0.0f));
        Flare.Set(new Color(1.0f, 1.0f, 1.0f, 1.0f));
        Flare.Set(new Rotation(0.0f, 1.0f, 0.0f, 0.0f));
        Flare.Set(new Texture("assets/sprites/flare.png"));
    ...
    public override void Update(float deltaTime, int screenWidth, int screenHeight)
    {
        ...
        if(MushroomSpawner.ShouldSpawn)
        {
            ...
            if(MushroomSpawner.SpawnedMushrooms < 5)
            {
        ...
        */
        ...
        Flare.Mutate<Position>((ref Position p) =>
        {
            p.X = (screenWidth / 100.0f) * 33.0f;
            p.Y = (screenHeight / 100.0f) * 90.0f;
            p.Z = 100.0f;
        });
        Flare.Mutate<Dimension>((ref Dimension d) =>
        {
            d.Width = (screenWidth / 100.0f) * 15.0f;
            d.Height = (screenHeight / 100.0f) * 25.0f;
        });
        Flare.Mutate<Color>((ref Color c) =>
        {
            c.R = 1.0f;
            c.G = 1.0f;
            c.B = 1.0f;
    ...
    private void CreateLayer(string name, float zIndex)
    {
        ...
        layer.Set(new Position(0.0f, 0.0f, zIndex));
        layer.Set(new Dimension(0.0f, 0.0f));
        layer.Set(new Color(1.0f, 1.0f, 1.0f, 1.0f));
        layer.Set(new Rotation(0.0f, 1.0f, 0.0f, 0.0f));
        layer.Set(new Texture(name));
        layer.Set(new Type2D(12));
        layers.Add(new Tuple<Entity, float>(layer, zIndex));
    }

    private void UpdateLayers(int screenWidth, int screenHeight) 
    {
        // Scale and position the layers based on the screen size
        foreach (var layer in layers)
        {
            layer.Item1.Mutate<Position>((ref Position p) =>
            {
                p.X = screenWidth / 2.0f;
                p.Y = screenHeight / 2.0f;
                p.Z = layer.Item2;
            });
            layer.Item1.Mutate<Dimension>((ref Dimension d) =>
            {
                d.Width = screenWidth / 2.0f;
                d.Height = screenHeight / 2.0f;
            });
        }
    }
}