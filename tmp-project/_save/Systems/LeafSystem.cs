...
public sealed class FallingLeavesSystem
{
    ...
    // --- configuration ---
    public int MaxLeaves = 80;
    public Dimension SpawnAreaSize = new Dimension(600.0f, 200.0f); // local units around Origin
    public float RecycleMargin = 2f;
    public RangeF FallSpeed = new RangeF(4.0f, 6.8f);
    public RangeF BaseDriftX = new RangeF(-0.5f, 0.5f);
    ...
    void CreateLeaf(bool initialRandomY)
    {
        ...
        entity.AddComponent(new Position(0, 0, leaf.Pos.z));  // will be set right below
        entity.AddComponent(new Dimension(32.0f, 32.0f));
        entity.AddComponent(new Color(1.0f, 1.0f, 1.0f, 1.0f));
        entity.AddComponent(new Rotation(0.0f, 1.0f, 0.0f, 0.0f));
        entity.AddComponent(new Texture(textures[_rng.Next(textures.Count)]));
        entity.AddComponent(new Type2D(12));
        leaf.Entity = entity;
    ...
    static void UpdateEntityRotation(in Leaf leaf, float deg)
    {
        ...
        leaf.Entity.Update(new Rotation(0f, 0f, s, c));
    }
